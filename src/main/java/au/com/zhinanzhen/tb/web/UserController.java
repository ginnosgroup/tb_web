...
public class UserController extends AbstractController {
	...
	public void WeiXinResult(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 注册并且返回
		if (!userService.addUser("", "", "", ThirdTypeEnum.WECHAT.toString(), "", openId, nickName, logo,
				recommendOpenId)) {
	...
	public Response<Boolean> addWechatUser(@RequestParam(value = "familyName", required = false) String familyName,
		...
		try {
			if (userService.isPhoneExitst(phone))
				return new Response<Boolean>(1, "手机号已注册", Boolean.FALSE);
			HttpSession session = req.getSession();
			if (session == null)
...
//					ThirdTypeEnum.WECHAT.toString(), phone, wechatId, wechatId, null, null));
			...
			return new Response<Boolean>(1, e.getMessage(), Boolean.FALSE);
		}
	}
